...
mod canary_test {
    ...
    async fn test_client_get_services() -> Result<(), ClientError> {
        ...
        let kubernetes_service = services.items.iter().find(|i| i.metadata.name == "kubernetes");
    ...
    async fn test_client_secrets() -> Result<(), ClientError> {
        ...
        let client = K8Client::default().expect("cluster could not be configured");
        let secrets = client.retrieve_items::<SecretSpec,_>(NameSpace::All).await?;

        let system_secrets: Vec<K8Obj<SecretSpec>> = secrets.items.into_iter().filter(|s| s.metadata.namespace == "kube-system").collect();

        info!("system secrets: {} has been retrieved",system_secrets.len());
