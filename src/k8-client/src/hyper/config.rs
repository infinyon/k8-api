...
impl ConfigBuilder for HyperClientBuilder {
        ...
        where P: AsRef<Path>
    {
        ...
        if let Err(_) = &mut self.0.root_store.add_pem_file(&mut rd) {
            Err(IoError::new(ErrorKind::InvalidInput,"problem loading root certificate".to_owned()))
        } else {
            Ok(self)
        }
    }

    fn load_client_certificate<P>(mut self,client_crt_path: P,client_key_path: P) -> Result<Self,IoError>
        where P: AsRef<Path> {
            ...
            &mut self.0.set_single_client_cert(client_certs, private_keys.remove(0));
