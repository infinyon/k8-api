...
impl <B>ClientConfigBuilder<B> where B: ConfigBuilder {
    ...
    fn config(config: &K8Config) -> Result<(B,Option<String>),IoError>  {
        ...
        let builder = B::new();
        match config {
            K8Config::Pod(pod_config) => Ok((Self::configure_in_cluster(builder,pod_config)?,None)),
            K8Config::KubeConfig(kube_config) => Self::configure_out_of_cluster(builder,&kube_config.config),
        }
    }
    ...
    fn configure_out_of_cluster(builder: B,kube_config: &KubeConfig) -> Result<(B,Option<String>),IoError>  {
        ...
        if let Some(ca_data) = &current_cluster.cluster.certificate_authority_data {
            ...
            let random_file_name =format!("eks-pem-{}",rand::thread_rng()
            ...
            if let Some(exec) = &current_user.user.exec {
                ...
                debug!("token: {}",String::from_utf8_lossy(&token_output.stdout).to_string());
                let credential: K8Obj<ExecCredentialSpec> = serde_json::from_slice(&token_output.stdout)?;
            ...
            let builder = if let Some(client_crt_path) = current_user.user.client_certificate.as_ref() {
                if let Some(client_key_path) = current_user.user.client_key.as_ref() {
                    debug!("loading client crt: {} and client key: {}",client_crt_path,client_key_path);
                    builder.load_client_certificate(client_crt_path,client_key_path)?
                } else {
                    return Err(IoError::new(ErrorKind::InvalidInput,"no client cert key path founded".to_owned()))
                }
            } else {
                return Err(IoError::new(ErrorKind::InvalidInput,"no client cert crt path founded".to_owned()))
            };
